The original implementation with notes for the MaCVi25 challenge can be found here: [YOLOv7 with distance estimation](https://github.com/Ben93kie/YOLOv7-DL23/tree/distance_network) 
...
```
Epoch  GPUMem  train_box  train_obj  train_cls  train_dist  total  labels  img_size  precision  recall  map0.5  map.5:.95  dist_err combined_metric val_box  val_obj  val_cls  val_dist
```
...
To produce a final score which takes the object detection performance and the distance error into account, the combined metric is specified as:
