...
def detect(save_img=False):
    ...
    #     model_weights=Path('osnet_ain_x1_0_msmt17.pt'),  # which ReID model to use
    #     device='cuda:0',
    #     fp16=False,
    #     per_class=False,
    #     det_thresh=0.05,
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        with torch.no_grad():   # Calculating gradients would cause a GPU memory leak
            detect_pred, distance_pred, heading_pred = model(img, augment=opt.augment)[0]
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                updated_dets = tracker.update(np.array(det[:,:-3].cpu()), im0)  # --> M X (x, y, x, y, id, conf, cls, ind)
