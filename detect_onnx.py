...
# Letterbox-Funktion (angepasst für Video-Frames)
...
# Video öffnen (0 = Webcam oder Pfad zu .mp4 Datei)
...
while True:
    ...
    # OpenCV NMS (optional, um doppelte Boxen zu entfernen)
    ...
    for i in indices:
        ...
        distance = int(rest * 1e6)
        heading = int(round((rest * 1e9) % 1000))

        # zurückrechnen auf Originalbild
        box = np.array([x0, y0, x1, y1])
        box -= np.array(dwdh * 2)
        box /= ratio
        box = box.round().astype(int).tolist()

        name = f"{names[cls_id]} {conf} {distance}m {heading}°"
    ...
    if cv2.waitKey(1) & 0xFF == 27:  # ESC drücken zum Beenden
        break
