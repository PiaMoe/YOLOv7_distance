...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    nc = prediction.shape[2] - 7  # number of classes bc of distance and heading 7 instead of 5
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf
        if nc == 1:
            x[:, 5:-2] = x[:, 4:5] # for models with one class, cls_loss is 0 and cls_conf is always 0.5,
        ...
        if multi_label:
            ...
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float(), x[i, -2, None]), 1)
        else:  # best class only
            conf, j = x[:, 5:-2].max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(), x[:,-2:]), 1)[conf.view(-1) > conf_thres]
