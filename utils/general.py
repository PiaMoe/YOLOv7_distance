...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    nc = prediction.shape[2] - 8  # number of classes bc of distance and heading 8 instead of 5
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            l = labels[xi]
            v = torch.zeros((len(l), nc + 8), device=x.device)
            v[:, :4] = l[:, 1:5]  # box
            v[:, 4] = 1.0  # conf
        ...
        # Compute conf
        if nc == 1:
            x[:, 5:-3] = x[:, 4:5] # for models with one class, cls_loss is 0 and cls_conf is always 0.5,
        ...
        # Detections matrix nx6 (xyxy, conf, cls, distance, heading)
        if multi_label:
            i, j = (x[:, 5:-3] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float(), x[i, -3, None]), 1)
        else:  # best class only
            conf, j = x[:, 5:-3].max(1, keepdim=True)
            x = torch.cat((box, conf, j.float(), x[:,-3:]), 1)[conf.view(-1) > conf_thres]
