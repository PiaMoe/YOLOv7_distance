...
detection model including two additional heads for distance and heading prediction. For each
anchor, it additionally predicts the normalized metric distance and both $$\sin(\theta)$$ and $$\cos(\theta)$$, where $$\theta$$ is the heading angle of the object/ boat. These three values are concatenated with the default YOLOv7 Detection Vector containing class predictions, objectness and boundingbox dimensions.
During inference, the normalized distance is rescaled according to the defined maximum distance. The heading value is computed by $$\theta = atan2(\sin(\theta), \cos(\theta))$$.
...
*Does not work currently for multiple head model.*
