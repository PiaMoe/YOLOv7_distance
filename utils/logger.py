...
def log_predictions(tensor, epoch, batch_i, output_dir, sample_prob=0.001, col_names=None):
    ...
    Args:
        ...
        col_names (list[str]): Optional, e.g., ["x", "y", "w", "h", "obj", "class_logits", "distance"]
    """
...
def safe_read_csv(file):
    ...
    return pd.DataFrame(rows, columns=["x", "y", "w", "h", "obj", "class_0", "distance"])


def evaluate_logs(csv_dir):
    # take all CSV files in directory
    ...
    for epoch, df in epoch_data.items():
        ...
        columns = ["x", "y", "w", "h", "obj", "class_0", "distance"]
